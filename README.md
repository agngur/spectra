...
Script which convert all .fits (binary) files to .asc (text files) in current directory. Extract main table from SDSS 1d spectra .fits files and save in asci text file (so You can also plot it simple by gnuplot). (Column of asci files are same as rows in .fits). 
